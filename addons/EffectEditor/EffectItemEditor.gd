@tool
extends Control
class_name EffectItemEditor;

var current_effect : Effect;
var path : String;
var editor : EditorInterface;

func display_effect(effect : Effect, path : String, editor : EditorInterface):
	current_effect = effect;
	self.path = path;
	self.editor = editor;
	$HBoxContainer/Label.text = str(path.get_file());


func remove():
	get_parent().remove_child(self);
	queue_free();


func set_even():
	$BG.modulate = Color(0, 0, 0, 0.2);


func _on_select_button_pressed():
	if editor != null:
		editor.select_file(path);
		editor.get_inspector().resource_selected.emit(current_effect, path)


func _on_delete_button_pressed():
	if editor != null:
		DirAccess.remove_absolute(ProjectSettings.globalize_path(path));
		var system = editor.get_resource_filesystem();
		system.filesystem_changed.emit();
		system.scan();
